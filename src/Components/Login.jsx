...
export default function Login() {
  ...
  useEffect(() => {
    // Simulate template-loader.js header/footer placeholder logic if needed
  }, []);
    ...
    <div className="bg-gray-50 min-h-screen font-[Poppins]">
      <main className="container mx-auto px-4 py-12">
        <div className="max-w-md mx-auto bg-white p-8 rounded-lg shadow-md">
          ...
          <div className="mb-6">
            <h3 className="text-sm font-medium text-gray-500 mb-2">Select your role</h3>
            <div className="grid grid-cols-3 gap-2">
              {['individual', 'corporate', 'therapist'].map((type) => (
                <button
                  key={type}
                  type="button"
                  onClick={() => handleRoleClick(type)}
                  className={`py-2 px-4 border rounded-md hover:bg-blue-50 hover:border-blue-300 ${
                    userType === type ? 'bg-blue-50 border-blue-300' : 'border-gray-300'
                  }`}
                >
                  {type.charAt(0).toUpperCase() + type.slice(1).replace('User', ' User')}
                </button>
              ))}
            </div>
          </div>
          ...
          <form className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 px-6 rounded-xl text-xl font-semibold shadow-lg transition-all duration-200 transform hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-blue-300/50"
            >
              <i className="fas fa-sign-in-alt mr-2"></i> Login to Your Account
            </button>
